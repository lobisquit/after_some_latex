...
\subsection{Classes of states}
	...
	\begin{theorem}[4.2, KT p. 95 (sufficient condition for recurrence)]
		An irreducible aperiodic \gls{mc}  is recurrent if:
	...
	\begin{proof}
		...
		\proofpart
			\begin{equation}\begin{split}
				...
				& \stackrel{(3)}{=} \ldots = \sum_{j=0}^{+\infty} Y_j \tilde{P}_{ij}^{(m)} \quad\forall i,\forall m
			\end{split}\end{equation}
			where
			\begin{itemize}
		...
		\proofpart
			...
			Since we have assumed that chain is transient we can state that $\tilde{\pi}_i(C_0) < 1$, then we can choose $\epsilon < \frac{1-\tilde{\pi}_i(C_0)}{Y_i}$ and then:
			\begin{equation*}
				1 - \tilde{\pi}_i(C_0) \le \epsilon Y_i < \frac{1-\tilde{\pi}_i(C_0)}{Y_i} Y_i \Rightarrow 1 - \tilde{\pi}_i(C_0) \le 1 - \tilde{\pi}_i(C_0)
			\end{equation*}
			Since this absurd, $\tilde{\pi}_i(C_0) = f_{i0} = 1$
