...
\subsection{Classes of states}
  ...
  \end{proof}
	...
	\begin{theorem}[4.2, KT p. 95 (sufficient condition for recurrence)]
		...
		\begin{equation}\noindent
			\exists \text{ a sequence } Y_i :  \text{, for } i = 1, \dots \\
			\begin{cases}
 				\sum\limits_{i=0}^{+\infty} P_{ij} Y_j \le Y_i 	& \forall i \neq 0 \\
	...
	\begin{proof}
		...
		\proofpart
			\begin{equation}\begin{split}
				...
				& \stackrel{(3)}{=} \ldots = \sum_{k=0}^{+\infty} Y_k \tilde{P}_{ik}^{(m)} \quad\forall m
			\end{split}\end{equation}
			where
			\begin{itemize}
