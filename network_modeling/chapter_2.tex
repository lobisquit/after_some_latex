...
\begin{equation}\begin{split}
q \cdot \underbrace{(u_k - u_{k-1})}_{X_k} &= p \cdot \underbrace{\left(u_{k+1} -u_k\right)}_{X_{k+1}} \\
...
Now we suppose that the system is in the \textit{hunt state} $H$ and it is looking at the $n^{th}$ bit of the slot. We call $P_{TL}(n)$ the probability of move to state $TL_1$ given that the system is looking at $n^{th}$ bit of the slot.
\begin{equation}
...
\end{equation}
In \eqref{probability_falselock}, the first sum regards the bits numbered from $n$ to $N-1$ wherease the second regards the bits numbered from $1$ to $n-1$.
...
\begin{definition}[n-step probability from i to j]
	We define the probability of going from state $i$ to state $j$ in $n$ steps, $\prob[\theta_{ij}=n]$, as follows:
	\begin{equation}\begin{split}
		\prob[\theta_{i j}=n] &= f_{i j}(n)=\prob[X_n=j,X_m \neq j | X_0 = i] \quad \text{with m=1, \dots , n-1}\\
		f_{ij}(0)&=0 \quad \forall i,j \,\, \text{ because $n \ge 1$}\\
		f_{ij}(n)&=P_{ij} \cdot \delta(n-1) + \sum\limits_{k \neq j} P_{i,k} \cdot f_{kj}(n-1)\\
	\end{split}\end{equation}
\end{definition}
...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    %\path [postaction={draw, {>[sep=5pt,reversed]}-{<[sep=5pt]}}, draw] (1) -- (2) node [very near start, my label] {$a$}  node [very near end, my label] {$\beta$} ;
		\path[-stealth] (2.south west)  edge[bend left=90] node [below] {$b$} (1.south east);
		\path[-stealth] (1.north east)  edge[bend left=90] node [above]  {$a$} (2.north west);
  ...
  \end{tikzpicture}
	...
	\label{fig:2sMC}
\end{figure}
For a two state \gls{mc} like the one in \Fig{fig:2sMC}, we can write:
\begin{equation}\begin{split}
...
\begin{equation}\begin{split}
	\exp[\theta_{01}] &= 1 + P_{00} \cdot \exp[\theta_{01}] \quad \exp[\theta_{01}] = \frac{1}{1-P_{00}} = \frac{1}{a}\\
	\exp[\theta_{10}] &= 1 + P_{11} \cdot \exp[\theta_{10}] \quad \exp[\theta_{10}] = \frac{1}{1-P_{11}} = \frac{1}{b}\\
	\exp[\theta_{00}] &= 1 + P_{01} \cdot \exp[\theta_{10}] =1+\frac{a}{b} = \frac{1}{1-P_{00}} = \left(\frac{b}{a+b}\right)^{-1}\\
	\exp[\theta_{11}] &= 1 + P_{10} \cdot \exp[\theta_{01}] =1+\frac{b}{a} = \frac{1}{1-P_{00}} = \left(\frac{a}{a+b}\right)^{-1}\\
	\lim_{n \to +\infty} P^n &=
	\begin{pmatrix}
...
\begin{equation} \begin{split}
	\exp[\theta_{01}^2] &= \frac{2 \cdot \exp[\theta_{01}]-1}{1-P_{00}} = \frac{\frac{2}{a}-1}{a} = \frac{2}{a^2} - \frac{1}{a}\\
...
\begin{definition}[$W_{i j}^{(n)}$ as n-step probability to reach j from i]
	\begin{equation}
		W_{i j}^{(n)} = \exp\left[\sum\limits_{l=0}^n \Chi\{ X_l=j\}|X_0=i\right] = \sum\limits_{l=0}^n\exp\left[\Chi \{ X_l=j\}|X_0=i\right]\sum\limits_{l=0}^n P_{i j}^{(l)}
	\end{equation}
\end{definition}
...
\begin{equation}
	\sum\limits_{j=0}^{r-1}W_{i j} = \exp\left[\sum\limits_{n=0}^{T-1}\sum\limits_{j=0}^{r-1}\Chi\{X_n=j\}|X_0=i\right] = \exp[T|X_0=i]=v_i
\end{equation}
...
\section{Steady-state probabilities}
	...
	\begin{theorem}[$\pi$ limiting distribution]
...
\section{Classes of states}
	...
	\begin{definition}[Number of returns on recurrent and transient states]
	...
	\begin{theorem}[Recurrence on communicant states]
  ...
  \end{proof}
	...
	\begin{center}
		\begin{tabular}{|c||c|c|c|}
			...
			$m_i = \sum\limits_{n=1}^{+\infty} n \cdot f_{ii}^{(n)}$ & $\infty$ & $\infty$ & $<\infty$ \\ \hline
	...
	\end{proof}
\section{G/M/1}
Let's define some parameters before analyzing this queueing system where arrivals are generic and there is only one server which time has a Markov distribution. Given $X$ a sequence of arrivals, we have:
\begin{description}
	\item[$t_n$:]  the time of the n-th arrival
	\item[$X_n$=] $X(t_n^{-})$ the arrival at time $t_n$
	\item[$a_k$:] the k-th service time, i.i.d. with exponential distribution
	\item[T:] interarrival time with distribution $G(t)$
\end{description}
We now would like now to calculate $P_{i~ i+1-j}$:
\begin{esp}
	P_{i~ i+1-j}&\stackrel{j<i+1}{=}\prob[\text{j depart. in an interarrival time}] \\
	&=\prob\left[\sum\limits_{k=1}^j a_k \le T \le \sum\limits_{k=1}^j a_k\right]\\
	&= \prob[\text{exactly j Poisson events in T}] = \int_0^{+\infty} \frac{e^{-\mu t} (\mu t)^j}{j!} dG(t)
\end{esp}
The case where everyone departs, or where there are at least j Poisson events can be represented as $P_{i 0}$
\begin{equation}
	P_{i 0} = \int_0^{+\infty} \sum\limits_{k=i+1}^{+\infty}\frac{e^{-\mu t} (\mu t)^k}{k!} dG(t) = 1- \int_0^{+\infty} \sum\limits_{k=0}^{i}\frac{e^{-\mu t} (\mu t)^k}{k!} dG(t)
\end{equation}
Which gives an almost lower triangular transition matrix.

\subsection{Data protocols - Layer 2 and 4}
\begin{figure}
	\input{img/diagProto.tex}
	\caption{A simple queueing protocol like the one explained in this section}
	\label{fig:dataProto}
\end{figure}
Let's now suppose to have a system where every T seconds some amount of data $\xi_n$ is generated and let $a_k=\prob[\xi_n=k]$. Let's now assume that the data is generated as an i.i.d. process.  \Fig{fig:dataProto} can be useful.
Suppose that the system schedule to send an amount of data at the beginning of each slot. Let's now analyze three protocols:
\begin{enumerate}
	\item all data is sent up to a max of M data unit
	\item data is sent if there is at least m data unit \label{proto2}
	\item same as 2. but send up to M data unit
\end{enumerate}
Let's now suppose that the data incoming in the during one slot is queued for the next transmission
\subsubsection*{Proto 1}
\begin{equation}
	X_{n+1}=\begin{cases}
			\xi_n & \text{if } X_n\le M\\ X_n-M+\xi_n & \text{if } X_n> M
	\end{cases}
\end{equation}
\paragraph{Transition diagram and matrix}
\begin{equation}
...
\end{equation}
\begin{figure}[ht]
	\label{fig:diag-proto1}
	\input{img/proto1.tex}
	\caption{Transition diagram for protocol 1}
\end{figure}

\subsubsection*{Proto 2}
\begin{equation}
	X_{n+1}=\begin{cases}
			\xi_n & \text{if } X_n\ge m\\ X_n+\xi_n & \text{if } X_n<m
	\end{cases}
\end{equation}
\paragraph{Transition diagram and matrix}
\begin{equation}
	P=\begin{blockarray}{cccccc}
			\begin{block}{c(ccccc)}
				1 & a_0 & a_1 & \dots & \dots & \dots\\
				2 & 0 & a_0 & \ddots & \dots & \dots\\
				\vdots & \vdots &\ddots &\ddots & \dots& \dots\\
				m-1 & 0 & \dots & 0 & a_0  & \dots\\
				m & a_0 & a_1 & a_2 &\dots & \dots\\
				m+1 & a_0 & a_1 & a_2 &\dots & \dots\\
				\vdots & \vdots &\vdots &\vdots & \vdots& \vdots\\
		\end{block}
	\end{blockarray}
\end{equation}
\begin{figure}[h]
	\label{fig:diag-proto2}
	\input{img/proto2.tex}
	\caption{Transition diagram for protocol 2}

\end{figure}
\subsubsection*{Proto 3}
\begin{equation}
	X_{n+1}=\begin{cases}
			\xi_n & \text{if } m\le X_n\le M\\
			 X_n+\xi_n & \text{if } X_n<m\\
			 X_n-M+\xi_n & \text{if } X_n> M
	\end{cases}
\end{equation}
\paragraph{Transition diagram}
The transition diagram is the combination of \Fig{fig:diag-proto1} and \Fig{fig:diag-proto2}.
%missing this diagram, but I don't have time right now
	...
	\begin{equation*}
		...
		\end{split}
	\end{equation*}
	We have to solve $\beta = A(\beta)$: %minding the condition $\beta \in (0;1)$
	\begin{enumerate}
	...
	\begin{figure}[h!]
		...
		\label{fig:cBetaK}
	\end{figure}
	If 2 is $A(\beta)$ there is no conclusion (no intersection). If, instead, 1 is $A(\beta)$ there is one intersection which is the solution and the chain is positive recurrent with $\pi_k = (1-\beta^*)(\beta^*)^k$.[\Fig{fig:cBetaK}]\\
...
We discussed, so far, only an aperiodic MC: the case where $P_{ii}^{(n)}=0$ if n is not a multiple of the period implies no convergence, which means that the MC has an oscillatory behaviour. Let's now consider $d$ the period s.t. $P_{ii}^{(m \cdot d)}\neq 0~\forall m\in \N $ (we take into account only positive values). The average first return time ($m_i$) can be calculated as
\begin{equation}
	\lim_{m\to+\infty} P_{ii}^{(m \cdot d)} = \frac{d}{m_i}
\end{equation}
We can ignore the $d$ value if we calculate:
\begin{equation}
	\lim_{n\to+\infty}\left[\frac{1}{n}\sum\limits_{k=0}^{n-1} P_{ii}^{(k)}\right] = \frac{1}{m_i}
\end{equation}

\begin{definition}[Entering in class via a state $i$ probability] \label{def:falling_probability}
	...
	\begin{proof}
		...
		\begin{itemize}
			...
			\item[$\alpha, ~\beta,~ \gamma$] are shown in \Fig{fig:th3_1}
		\end{itemize}
	...
	\begin{proof}
		...
		Since the sum of the $\pi_j$ is not zero, at least one of them must be strictly positive.
		Corresponding states are then positive recurrent, proving our theorem.
	\end{proof}
	...
	\begin{definition}[Staying in a class probability]
	...
	\begin{theorem}[$Y_i(n)$ is monothonically non increasing]
	...
	\begin{lemma}[4.13 (Ross 2, pg. 78-82)] \label{lemma:MC_irreducible_fi0}
		...
		$$ \forall i \neq 0, f_{i 0} = 1 $$
		where $f_{i 0}$ is the probability of reaching state $0$ starting from $i$ at any time in the future.
	\end{lemma}
	---
...
We can then write the transform of the $\pi$ as
\begin{esp}
	\Pi(s) &= \pi_0 \cdot A(s) + \frac{A(s) \cdot \Pi(s) - \pi_0 a_0}{s} -\frac{A(s) \cdot \pi_0 - \pi_0 a_0}{s} \\
	s \cdot  \Pi(s) &- A(s) \cdot \Pi(s) = \pi_0 \cdot A(s) \cdot (s-1) \\
	\Pi(s) &= \pi_0 \cdot \frac{A(s)\cdot(s-1)}{s - A(s)}
\end{esp}
Letting $s$ go to $1$ and reminding that $A(1)=1$ we can write
\begin{esp}\label{eq:s-to1}
	1=\lim_{s\to 1} \Pi(s) &= \pi_0 \cdot \lim_{s\to 1} \frac{s-1}{s-A(s)} \\
	&=\pi_0 \cdot \lim_{s\to 1} \left(1-\underbrace{\frac{A(s)-1}{s-1}}_{(\bigstar)}\right)^{-1} = \pi_0\cdot\left[1-A^{\prime}(1)\right]^{-1} = \frac{\pi_0}{1-A^\prime(1)}
\end{esp}
...
\begin{esp} \label{eq:pi0rho}
	1&= \frac{\pi_0}{1-\rho} \\ \Rightarrow & \pi_0 = 1-\rho
\end{esp}
...
The following table summarize the results we just found %\Tab{tab:summ}
\begin{table}[h]
\centering
\label{tab:summ}
\begin{tabular}{c|c|c}
...
Transient                                                     & $< +\infty$                                                                             & $+\infty$                                                            \\ \hline
\begin{tabular}[c]{@{}c@{}}Positive \\ recurrent\end{tabular} & $+\infty$                                                                               & $< +\infty$                                                          \\ \hline
...
We can then write:
\begin{esp}
	\underbrace{\left(p_i + q_i\right)}_{=1} \cdot Z_i &= p_i \cdot Z_{i+1} + q_i \cdot Z_{i-1} \quad i>1 \\
	q_i \cdot\left(Z_i - Z_{i-1}\right) &=p_i \cdot\left( Z_{i+1} - Z_i \right)\\
	Z_{i+1} &= Z_i + \frac{q_i}{p_i}\cdot \underbrace{\left(Z_i - Z_{i-1}\right)}_{(\spadesuit)}
\end{esp}
...
\subsubsection{Random walk analysis with G/M/1}
...
\begin{equation}
	1-s = \sum\limits_{j=0}^{+\infty} a_{j} \cdot (1-s^j) \stackrel{(\clubsuit)}{=}\sum\limits_{j=1}^{+\infty} a_{j} \cdot (1-s^j) = 1-A(s)
\end{equation}
where $(\clubsuit)$ is possible as $s^0=1$, so the first term of the sum is null.
...
\begin{esp} \label{eq:zirho}
	z_i \ge \sum\limits_{j=0}^{+\infty} P_{i j} \cdot z_j &= a_0 \cdot z_i-1 + a_1 \cdot z_i + \dots \\ &= \sum\limits_{j=0}^{+\infty} a_{j} \cdot z_{i-1+j} ~ i\neq 0
\end{esp}

For $z_i=i$ we now wonder when the relation \eqref{eq:pi0rho} holds:
\begin{esp}
	i &\ge \sum\limits_{j=0}^{+\infty} a_{j} \cdot (i-1+j) =(i-1)\cdot\sum\limits_{j=0}^{+\infty} a_{j} + \sum\limits_{j=0}^{+\infty} a_{j} \cdot j \stackrel{(\diamondsuit)}{=} i-1+\rho \\
	i &\stackrel{?}{\ge } i-1+\rho \quad \text{true if }\rho \le 1
\end{esp}
where ($\diamondsuit$) is derived from \eqref{eq:MG1-intro} . Recalling that the steady state probability is positive and equation \eqref{eq:pi0rho}, we can derive the table \ref{tab:rhoBehavior}:

\begin{table}[h!]
\centering
\caption{Behavior of the \gls{mc} when varying $\rho$}
\label{tab:rhoBehavior}
\begin{tabular}{c|c|c}
