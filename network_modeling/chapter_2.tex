...
Suppose that the system schedule to send an amount of data at the beginning of each slot. Let's now analyze two protocols:
\begin{enumerate}
...
The transition diagram is the combination of the two previous protocols.
...
\end{esp}
where ($\diamondsuit$) is derived from \eqref{eq:MG1-intro} . Recalling that the steady state probability is positive and equation \eqref{eq:pi0rho}, we can derive the table \ref{tab:rhoBehaviour}:
