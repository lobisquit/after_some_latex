...
\begin{equation}\begin{split}
	...
	&\stackrel{1}{=}+ \sum\limits_{k \neq j} \exp[\theta_{k,j}] \cdot P_{i,k} \forall i \neq j\\
	& \text{We then get $N$ equations in $N$ unknowns, so the system can be solved.}
\end{split}\end{equation}
The average return time is the inverse of the in-state remaining probability, as we can see in the example of
the two state MC
\begin{equation}\begin{split}
	\exp[\theta_{0,1}] &= 1 + P_00 \cdot \exp[\theta_{0,1}] \quad \exp[\theta_{0,1}] = \frac{1}{1-P_{00}} = \frac{1}{a}\\
	\exp[\theta_{1,0}] &= 1 + P_00 \cdot \exp[\theta_{0,1}] \quad \exp[\theta_{1,0}] = \frac{1}{1-P_{11}} = \frac{1}{b}\\
	\exp[\theta_{0,0}] &= 1 + P_00 \cdot \exp[\theta_{0,1}] =1+\frac{a}{b} = \frac{1}{1-P_{00}} = \left(\frac{b}{a+b}\right)^{-1}\\
	\exp[\theta_{1,1}] &= 1 + P_00 \cdot \exp[\theta_{0,1}] =1+\frac{b}{a} = \frac{1}{1-P_{00}} = \left(\frac{a}{a+b}\right)^{-1}\\
	\lim_{n \to +\infty} P^n &=
	\begin{pmatrix}
