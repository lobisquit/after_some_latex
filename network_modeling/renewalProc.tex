...
\subsection{Time intervals}
	...
	In this case, given $\beta_t = \delta_t + \gamma_t$, the join distribution of $\delta_t$ and $\gamma_t$ can be written as follows.
	...
	The asymptotic value of this expectation removes the transient behaviour of the process and gives this surprising result.
	\begin{equation*}
	...
	\end{equation*}
	This result is indeed correct, but it clashes with our previous knowledge of the mean interarrivals time for Poisson process.
...
\begin{equation}
	\begin{cases}
		S_n / n\xrightarrow{n\rightarrow\infty}\mu = \exp[X] \quad &\text{with probability 1} \\
...
\begin{proof}
	In have infinite time, we have a finite number of renewals if and ony if there exists an event that is the last one, of as
		\begin{equation}
			\begin{split}
