...
\subsection{Examples}
	...
	\eeq
	where $\mu^c = \int_0^c(1-F(x))\,dx$, so as $c \to \infty$ we see that $\mu^c \to \mu = \int_0^{+\infty}(1-F(x))\,dx$. [See \Fig{fig:mu}]
	\begin{figure}
	\centering
...
\beq
\pi_{G} = \frac{P_{10}(m)}{(\dots)} %brutto. riusciamo a trovare la formula?
\hspace{20mm}
