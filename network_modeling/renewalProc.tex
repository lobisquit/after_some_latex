...
\subsection{Examples}
	...
	\beq
	\prob[\text{0 errors in 3 pages}\footnote{not necessarily consecutive}] = e^{-1.2}
	\eeq
...
\begin{definition}[Point of increase]
	 Given a distribution function F we call \textit{point of increase} every point $\alpha$ such that
	\begin{equation}
...
\begin{definition}[Arithmetic function]
	 We say that a distribution function F is \textit{arithmetic} if $\exists \lambda > 0$ such that
	\begin{equation}
...
\begin{definition}[Directly Riemann Integrable Function]
	 Given a function g defined on [0, $\infty$) we define the following quantities for $\delta > 0$:
	\begin{align*}
...
\begin{theorem}[Key Renewal Theorem]
	It is given a distribution function F of a positive random variable $a$ with mean $\mu$. We suppose that $a$ is directly Riemann integrable and that $A$ is the solution of the \textit{renewal equation}
	\begin{equation}
...
\begin{theorem}[Key renewal theorem - v2]
	 Is given a distribution function F of a positive random variable with mean $\mu$. We suppose $h>0$ and that $M(t)=\sum_{k=1}^{+\infty} F_{k}(t)$ is the \textit{renewal function} associated with F.
	If F is not arithmetic then
	\begin{equation}
...
\begin{theorem}[Limiting distribution on irreducible semi-Markov process]
...
The answer is:
\beq
m_0 = P_{00}\cdot1+P_{01}\left[1+\frac{m}{P_{10}(m)}\right]
\label{Formula}
\eeq
...
\beq
\lim_{t \to \infty}\frac{R(t)}{t} = \frac{\vec{\pi}\vec{R}}{\vec{\pi}\vec{T}} = \frac{\pi_{00}\cdot 1}{\pi_{00}\cdot1+(1-\pi_{00})m}
\eeq
...
\end{bmatrix}
\eeq
Moreover, if the feedback channel is i.i.d., we have nothing to remember: regardless of whether the ack is good or bad, we do not care if the transmission was a failure $\rightarrow$ for this case we can use a simplified model with only $3$ states (\Fig{fig:3states}), where the metrics are:
\beq
R =
\begin{bmatrix}
1\\
...
\end{figure}
The throughput is:
\beq\label{eq:thrPig0}
\frac{\pi_{G_0}}{\pi_{G_0}+m(1-\pi_{G_0})}
\hspace{5mm}
\vec{\pi} = \vec{\pi}\cdot\vec{P}
\eeq
...
\eeq
So, rewriting \eqref{eq:thrPig0}, the throughput is:
\beq
throughput = \frac{(1-\delta)P_{10}(m)}{(1-\delta)P_{10}(m) + m[1-(1-\delta)P_{00}-\delta P_{00}(m)+\delta P_{10}(m)]}
\eeq
\subsection{Graph reduction}
What if we rewrite the scheme, such that we can reduce it like in \Fig{fig:GBN}?\\
...
\begin{esp}
\pi_{G} &= \frac{P_{10}(m)}{(1-\delta+\delta \cdot m)P_{10}(m)+m[(1-\delta)P_{01}+\delta P_{01}(m)]} \\ %brutto. riusciamo a trovare la formula?
\hspace{0.5cm}
\pi_{B} &= \frac{(1-\delta)P_{01}+\delta P_{01}(m)}{(1-\delta+\delta \cdot m)P_{10}(m)+m[(1-\delta)P_{01}+\delta P_{01}(m)]}
\end{esp}
Reward and time:
\beq
R =
...
\end{bmatrix}
\eeq
And finally we can estimate the throughput for the system in \Fig{fig:2states}:
\beq
\lim_{t \to \infty} \frac{R(t)}{t} = \frac{\vec{\pi}\vec{R}}{\vec{\pi}\vec{T}} = \frac{(1-\delta)P_{10}(m)}{(1-\delta+\delta \cdot m)P_{10}(m)+m[(1-\delta)P_{01}+\delta P_{01}(m)]}
\eeq
