	...
	\begin{esp}
		|\E_1(\r)|^2
			...
			& = |E_{oi}|^2
				...
				(1 + \rho^* ~ e^{-\jmath 2 \beta_1 x_1}) \\
			& = |E_{oi}|^2 (1 + |\rho|^2 + 2 |\rho| \cos(2 \beta_1 x_1+\phi_{\rho})) \\
	\end{esp}

	Questi piani individuano lungo $\hat{x}_3$ settori in cui l'impedenza vista dall'onda elettromagnetica varia. Essa, nel caso dell'onda piana, può essere semplicemente calcolata come

	\begin{esp}
		Z(x_1)
		& = \frac{E(x_1)}{H(x_1)}
			= \frac
				{E_{oi} (1 - e^{\jmath 2 \beta_1 x_1}) ~ e^{- \jmath \beta_1 x_1}}
				{-\frac{E_{oi}}{\eta_1} (1 + e^{\jmath 2 \beta_1 x_1}) ~ e^{- \jmath \beta_1 x_1} } \\
		& = \eta_1 ~ \frac
			{e^{\jmath \beta_1 x_1} - e^{\jmath \beta_1 x_1}}
			{e^{\jmath \beta_1 x_1} + e^{\jmath \beta_1 x_1}} \\
		& = \frac{\eta_1}{\jmath} ~ \frac{\sin(\beta_1 x_1)}{\cos(\beta_1 x_1)}
			= -\jmath \eta_1 \tan(\beta x_1)
	\end{esp}
