...
\subsection{Coefficienti di riflessione e trasmissione}
	...
	\begin{figure}[ht] \label{fig:campi_incidenza_normale_isolanti}
		\centering
...
\subsection{Interferenza in riflessione}
	...
	\begin{figure}[ht] \label{fig:campi_incidenza_non_normale_isolanti}
		\centering
	...
	%\begin{esp}
	%	\E_1(\r)
	%	& = \E_{oi} ~ e^{-j \k_i \cdot \r} + \E_{or} ~ e^{-j \k_r \cdot \r}
	%		E_{oi} \hat{x}_3 ~ e^{-\beta_1 ( \cos(\theta_i) x_1 + \sin(\theta_i) x_2} (1 + \rho ~ ^{- 2 \beta_1 ( \cos(\theta_i) x_1}
	%
	%\end{esp}
